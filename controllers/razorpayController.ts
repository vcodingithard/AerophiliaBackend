...
// ------------------------
export const verifyPayment = asyncHandler(async (req: Request<{}, {}, RazorpayVerifyParams & { registration_id: string; event_id: string }>, res: Response) => {
  const { razorpay_order_id, razorpay_payment_id, razorpay_signature, registration_id, event_id } = req.body;
  ...
  const isValid = verifyRazorpayPayment({ razorpay_order_id, razorpay_payment_id, razorpay_signature }, secret);
  ...
  res.status(200).json({ success: true, message: "Payment verified and registration updated successfully" });
});
